...
par(mfrow=c(2,2))
graph<-as.numeric(data_sub$Global_active_power)
plot(graph,type = "l", ylab="Global Active Power (kilowatts)", xaxt = "n", xlab="")
axis(1,at=c(0,length(data_sub$Date)/2,length(data_sub$Date)),labels=c("Thu", "Fri", "Sat"))

graph<-as.numeric(data_sub$Voltage)
plot(graph,type = "l", ylab="Voltage", xlab="datetime", xaxt = "n")
axis(1,at=c(0,length(data_sub$Date)/2,length(data_sub$Date)),labels=c("Thu", "Fri", "Sat"))
...
graph<-as.numeric(data_sub$Global_reactive_power)
plot(graph,type = "l", xlab="datetime", ylab="Global_reactive_power", xaxt = "n")
axis(1,at=c(0,length(data_sub$Date)/2,length(data_sub$Date)),labels=c("Thu", "Fri", "Sat"))
